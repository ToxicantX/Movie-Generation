param(
    ...
    [string]$WorkflowPath,
    [string]$ShotId = "TEST_SSJ_EP01_SC01_SH02",
    [string]$Mode = "image_to_video_with_storyboard_image_path",
    [string]$SubmitScript = "E:\workspace\ComfyUIProjects\Movie-Generation\scripts\submit_single_workflow_when_ready.ps1",
    [string]$WaitScript = "E:\workspace\ComfyUIProjects\Movie-Generation\scripts\wait_comfy_prompt.ps1",
    [string]$ResultPath = "E:\workspace\ComfyUIProjects\Movie-Generation\manifests\single_i2v_workflow_result.json",
    [int]$PollSeconds = 10,
    [int]$MaxPolls = 120
)
...
try {
    ...
    if ($submitExit -ne 0) {
        $result.status = "submit_not_ready_or_failed"
